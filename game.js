...
function create() {
    ...
    player = this.physics.add.sprite(config.width/2, config.height-100, 'sun').setCollideWorldBounds(true).setDepth(5);
    ...
    for(let i=1; i<=5; i++) { this.add.text(config.width-40, 150+(i*40), 'B'+i, {background:'#333', padding:4}).setInteractive().on('pointerdown', () => { level=i; prepareBoss.call(this); }); }
